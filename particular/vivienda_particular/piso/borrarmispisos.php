...
// Enviar consulta
$instruccion = "SELECT * FROM Pisos WHERE id_usuario = " . $_SESSION['id_usuario'];
    ...
    <div class="pisos-container">
        ...
        if ($nfilas > 0) {
            for ($i=0; $i<$nfilas; $i++) {
                ...
                echo "<form action='borrarmispisos2.php' method='POST' style='margin-top: 10px;'>";
                echo "<input type='hidden' name='id_piso' value='" . $resultado['id_piso'] . "'>";
                echo "<button type='submit' class='delete-button' onclick='return confirm(\"¿Estás seguro de que quieres eliminar este piso?\");'>Eliminar</button>";
                echo "</form>";
                echo "</div>";
            }
        } else {
