...
$consulta = "SELECT l.*, u.nombre as nombre_propietario
             ...
             AND l.id_usuario = " . $_SESSION['id_usuario'];
...
if ($local['tipo'] == 'alquiler') {
    $consulta_transaccion = "SELECT tla.*, u.nombre as nombre_arrendatario 
                            FROM Transaccion_local_alquiler tla 
                            JOIN Usuarios u ON tla.id_usuario_arrendatario = u.id_usuario 
                            WHERE tla.id_local = '$id_local'";
} else {
    $consulta_transaccion = "SELECT tlv.*, u.nombre as nombre_comprador 
                            FROM Transaccion_local_venta tlv 
                            JOIN Usuarios u ON tlv.id_usuario_comprador = u.id_usuario 
                            WHERE tlv.id_local = '$id_local'";
}
...
<head>
    <meta charset="UTF-8">
    <title>Junteate - Detalles de Mi Local</title>
    <style>
        body {
...
<body>
    ...
    <div class="local-container">
        <div class="local-card">
            ...
            <div class="local-precio"><?php echo htmlspecialchars($local['precio']); ?>€/mes</div>
            <div class="local-tipo"><?php echo ucfirst(htmlspecialchars($local['tipo'])); ?></div>
            <div class="estado-local">Estado: <?php echo $local['disponible'] == 'si' ? 'Disponible' : 'No disponible'; ?></div>

            <div class="botones-container">
                <form action="editarmislocales2.php" method="POST" style="display: inline;">
                    <input type="hidden" name="id_local" value="<?php echo $local['id_local']; ?>">
                    <input type="hidden" name="direccion" value="<?php echo htmlspecialchars($local['direccion']); ?>">
                    <input type="hidden" name="localidad" value="<?php echo htmlspecialchars($local['localidad']); ?>">
                    <input type="hidden" name="provincia" value="<?php echo htmlspecialchars($local['provincia']); ?>">
                    <input type="hidden" name="codigo_postal" value="<?php echo htmlspecialchars($local['codigo_postal']); ?>">
                    <input type="hidden" name="precio" value="<?php echo htmlspecialchars($local['precio']); ?>">
                    <input type="hidden" name="descripcion" value="<?php echo htmlspecialchars($local['descripcion']); ?>">
                    <input type="hidden" name="tipo" value="<?php echo htmlspecialchars($local['tipo']); ?>">
                    <input type="hidden" name="disponible" value="<?php echo htmlspecialchars($local['disponible']); ?>">
                    <input type="hidden" name="foto" value="<?php echo htmlspecialchars($local['foto']); ?>">
                    <button type="submit" class="editar-button">Editar</button>
                </form>

                <form action="borrarmislocales2.php" method="POST" style="display: inline;">
                    <input type="hidden" name="id_local" value="<?php echo $local['id_local']; ?>">
                    <button type="submit" class="borrar-button" onclick="return confirm('¿Estás seguro de que quieres eliminar este local?');">Borrar</button>
                </form>
            </div>
        </div>
        ...
        <a href="mislocales.php" class="volver-button">Volver a Mis Locales</a>
    </div>
</body>
</html>
<?php
