...
if (!$id_local || !$precio) {
    header("Location: localesalquilados.php");
    exit();
}

// Obtener información detallada del local
$consulta = "SELECT l.*, u.nombre as nombre_propietario, tla.fecha_transaccion
             FROM Locales l 
             JOIN Usuarios u ON l.id_usuario = u.id_usuario 
             JOIN Transaccion_local_alquiler tla ON l.id_local = tla.id_local
             WHERE l.id_local = '$id_local' 
             AND tla.id_usuario_arrendatario = " . $_SESSION['id_usuario'];
...
<head>
    <meta charset="UTF-8">
    <title>Junteate - Detalles de Local Alquilado</title>
    <style>
        body {
...
<body>
    ...
    <div class="local-container">
        <div class="local-card">
            ...
            <div class="fecha-alquiler">Alquilado el: <?php echo date('d/m/Y', strtotime($local['fecha_transaccion'])); ?></div>

            <div class="botones-container">
                <form action="procesaranularalquiler.php" method="POST" style="display: inline;">
                    <input type="hidden" name="id_local" value="<?php echo $local['id_local']; ?>">
                    <button type="submit" class="anular-button">Anular Alquiler</button>
                </form>

                <form action="contactar.php" method="POST" style="display: inline;">
                    <input type="hidden" name="id_local" value="<?php echo $local['id_local']; ?>">
                    <input type="hidden" name="id_propietario" value="<?php echo $local['id_usuario']; ?>">
        ...
        <a href="localesalquilados.php" class="volver-button">Volver a Locales Alquilados</a>
    </div>
</body>
</html>
<?php
